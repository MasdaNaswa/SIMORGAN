...
@section('content')
        ...
        <main class="flex-1 px-4 md:px-8 py-6 bg-[#F8FAFC]">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6 px-4 md:px-6 py-4">
                ...
                <div class="flex gap-2">
                    <button class="flex items-center gap-2 py-2 px-4 rounded transition focus:outline-none focus:ring-2 text-sm md:text-base
                        @if(isset($canAccess) && $canAccess)
                        bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-300" onclick="openAddModal()" @else
                            bg-gray-400 text-gray-200 cursor-not-allowed" onclick="openInfoAksesModal()" @endif
                        <span>Tambah</span>
                    </button>
                </div>
            </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            @else
                                <a href="{{ $pkData->previousPageUrl() }}"
                                    class="px-3 py-1 bg-white border border-gray-300 rounded-md text-sm hover:bg-gray-50">Sebelumnya</a>
                            @endif
                            ...
                            @if($pkData->hasMorePages())
                                <a href="{{ $pkData->nextPageUrl() }}"
                                    class="px-3 py-1 bg-white border border-gray-300 rounded-md text-sm hover:bg-gray-50">Selanjutnya</a>
                            @else
                                <span class="px-3 py-1 bg-gray-100 text-gray-400 rounded-md text-sm cursor-not-allowed"><i
    ...
    @include('components.opd.info-akses-pk-bupati')
...
@push('scripts')
    <script>
        ...
        // ================ FUNGSI UNTUK NOMOR OTOMATIS ================
        // Fungsi untuk mendapatkan nomor otomatis dari tabel
        function getAutoNumberFromTable() {
            let table = document.getElementById('pkBupatiTable');

            if (!table) {
                table = document.querySelector('.bg-white.shadow.rounded-lg table');
            }
        ...
        // Fungsi untuk mengisi nomor otomatis di modal tambah
        function setAutoNumberInModal() {
            const noInput = document.getElementById('no');
            if (noInput) {
                const nextNumber = getAutoNumberFromTable();
                noInput.value = nextNumber;
            } else {
                setTimeout(setAutoNumberInModal, 200);
            }
        }
        ...
        function openAddModal() {
            ...
            // Panggil fungsi untuk mengisi nomor otomatis
            setTimeout(setAutoNumberInModal, 300);
        }
            ...
            fetch(`/pk-bupati/show/${id}`)
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        document.getElementById('editSasaranStrategis').value = data.sasaranStrategis || '';
        ...
        function deleteData(id) {
            ...
            if (form) {
                form.action = `/pk-bupati/destroy/${id}`;
            }

            const hapusIdInput = document.getElementById('hapusId');
            if (hapusIdInput) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (form) {
                form.addEventListener('submit', function (e) {
                    ...
                    // Debug: cek nilai semua input
                    console.log('=== DATA YANG AKAN DIKIRIM ===');
                    console.log('no:', document.getElementById('no').value);
                    console.log('sasaranStrategis:', document.getElementById('sasaranStrategis').value);
                    console.log('indikatorKinerja:', document.getElementById('indikatorKinerja').value);
                    console.log('target2025:', document.getElementById('target2025').value);
                    console.log('satuan:', document.getElementById('satuan').value);

                    const formData = {
                        no: document.getElementById('no').value,
                        sasaranStrategis: document.getElementById('sasaranStrategis').value,
            ...
            if (editForm) {
                editForm.addEventListener('submit', function (e) {
                    ...
                    const formData = {
                        ...
                        sasaranStrategis: document.getElementById('editSasaranStrategis').value,
            ...
            // Update nomor otomatis saat filter tahun/semester berubah
